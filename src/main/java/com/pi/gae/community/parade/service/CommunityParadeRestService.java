...
public class CommunityParadeRestService {
  ...
  public PassFailResponse attachCommunity(AttachRequest request) throws Exception {
    ...
    Communities communities =
        FirebaseManager.getInstance().value("/data/community_metadata", Communities.class);

    boolean success = false;
    String error = "Community not found";
    for (Map.Entry<String, CommunityMetadata> entry : communities.getCommunities().entrySet()) {

      if (request.getCommunityId().equals(entry.getKey())) {
        FirebaseManager.getInstance().set("/data/users/" + request.getUserToken().getUserId()
  ...
  public CommunityResponse getCommunityFromCoordinates(@PathParam("latitude") String latitude,
  ...
  public CommunityResponse getCommunityFromUuid(@PathParam("uuid") String uuid) throws Exception {
    Communities communities =
        FirebaseManager.getInstance().value("/data/community_metadata", Communities.class);
  ...
  public Response getConfig(@HeaderParam("etag") String requestEtag) {
    ...
    api.setCommunityFromCoordinates(prefix + "/community/from-coordinates/{latitude}/{longitude}");
    ...
    return Response.status(status).header("if-none-match", etag).type(MediaType.APPLICATION_JSON)
        .entity(entity).build();
  }
  ...
  public Response userCreateAccount(CreateAccountRequest request) throws Exception {
    ...
    UserProfile profile = fb.value("/data/users/" + userId + "/profile", UserProfile.class);
    ...
    if (profile == null) {
      ...
      fb.set("/data/users/" + userId + "/password",
          ProvidenceCrypto.encryptPassword(request.getPassword()));


      entity =
          ProvidenceCrypto.generateUserToken(request.getFirstName(), request.getLastName(), userId);
  ...
  public FirebaseLoginTokenResponse userFirebaseToken(UserToken userToken) throws Exception {
  ...
  @POST
  @Path("/user/login")
  public UserToken userLogin(LoginRequest request) throws Exception {
    String userId = ProvidenceCrypto.createUserId(request.getUsername());
    FirebaseManager fb = FirebaseManager.getInstance();
    ...
    UserProfile profile = fb.value("/data/users/" + userId + "/profile", UserProfile.class);
    UserToken userToken = ProvidenceCrypto.generateUserToken(profile.getFirstName(),
